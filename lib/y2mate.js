 ...
 */
async function yt(url, quality, type, bitrate, server = 'en397') {
  let ytId = ytIdRegex.exec(url)
  url = 'https://youtu.be/' + ytId[1]
  let res = await post(`https://www.y2mate.com/${server}/youtube-mp3`, {
    url,
    q_auto: 0,
    ajax: 1
  })
  ...
  let res2 = await post(`https://www.y2mate.com/${server}/download-youtube`, {
    type: 'youtube',
    _id: id[1],
    v_id: ytId[1],
    ajax: '1',
...
module.exports = {
   ...
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  yta(url, resol = '128kbps', server = 'en397') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  /**
